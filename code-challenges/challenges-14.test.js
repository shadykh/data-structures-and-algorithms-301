...
const screenForNames = (arr) => {
  // Solution code here...
  let localArray = arr.filter(val => {
    let regixMr = /^(Mr. [A-Z])/g;
    let regixMrs = /^(Mrs. [A-Z])/g;
    let regixMs = /^(Ms. [A-Z])/g;
    let regixDr = /^(Dr. [A-Z])/g;
    let regixLetter = /[a-zA-Z]+/g;
    if (regixLetter.test(val) && (regixMr.test(val) || regixMrs.test(val) || regixMs.test(val) || regixDr.test(val))) {
      return true;
    }
  });
  return localArray;
...
let biggerThanLuke = (arr) => {
  ...
  let namesArray = [];
  arr.filter(val => {
    if (!(val.name === 'Luke Skywalker') && val.mass > 77) {
